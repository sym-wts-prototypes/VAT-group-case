...
export function LegalEntityModal({
  ...
  // Candidate VAT group representatives: sibling entities (same org) flagged as representative.
  const representatives = siblings.filter((s) => s.id !== entity?.id && s.vatGroupRepresentative);
    ...
    <div className="flex gap-2">
      {([["Yes", true], ["No", false]] as const).map(([label, val]) => (
        <button
          ...
          className={`rounded-full px-5 py-2 text-[14px] leading-[20px] transition-colors ${
            value === val ? "bg-primary text-white" : "bg-white border border-neutral-200 text-neutral-600 hover:bg-neutral-50"
          }`}
        >
          {label}
        </button>
