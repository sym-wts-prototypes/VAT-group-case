...
for (const id of ids) {
  ...
  // Canvas thumbnails (optional; generated by `pnpm snapshots`).
  const snaps = path.join(prototypesDir, id, 'snapshots')
  if (fs.existsSync(snaps)) {
    fs.cpSync(snaps, path.join(dest, 'snapshots'), { recursive: true })
  }
